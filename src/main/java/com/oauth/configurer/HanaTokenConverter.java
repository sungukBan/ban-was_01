...
public class HanaTokenConverter implements AccessTokenConverter {
...
private UserAuthenticationConverter userTokenConverter = new DefaultUserAuthenticationConverter();
	...
	public Map<String, ?> convertAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
		...
		*/
		...
		if (token.getScope()!=null) {
			String scopeStr = convertScope(token.getScope());
			response.put(scopeAttribute, scopeStr);
		}
	...
	/**
	 * 마이데이터 규격에 맞게 Scope를 변경
	 * 공백으로 Scope를 구분한다.
	 * @param scops
	 * @return
	 */
	public String convertScope(Set<String> scops) {
		StringBuffer sb = new StringBuffer();
		
		String[] arr = scops.toArray(new String[scops.size()]);
